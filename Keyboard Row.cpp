...
class Solution {
public:
    vector<string> findWords(vector<string>& v1) {
        string s1="qwertyuiop",s2="asdfghjkl",s3="zxcvbnm";
        v<string> res;
        for(auto &ele:v1)
        {
            bool flag=true;
            for(auto it:ele) if(s1.find(tolower(it))==string::npos)  {
                flag=false; 
                break;
            }
            
            if(flag) {res.pb(ele);continue;}
            flag=true;
            
            for(auto it:ele) if(s2.find(tolower(it))==string::npos)  {
                flag=false; 
                break;
            }
            ...
            for(auto it:ele) if(s3.find(tolower(it))==string::npos)  {
                flag=false; 
                break;
            }
