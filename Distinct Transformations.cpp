...
ll fun(string &s1,string &s2,ll i,ll j,v<v<ll>>&dp)
{
    ...
    else if(j==s2.sz()) rn dp[i][j]=fun(s1,s2,i+1,j,dp);
    else if(i==s1.sz()) rn dp[i][j]=0;
    else if(s1[i]!=s2[j]) rn dp[i][j]=fun(s1,s2,i+1,j,dp);
    else rn dp[i][j]=fun(s1,s2,i+1,j+1,dp) + fun(s1,s2,i+1,j,dp);
}
int32_t main()
{
    string s1,s2;cin>>s1>>s2;
    v<v<ll>> dp(s1.sz()+1,v<ll>(s2.sz()+1,-1));
    cout<<fun(s1,s2,0,0,dp)<<"\n";
    rn 0;
}
