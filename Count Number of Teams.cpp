...
class Solution {
public:
    int numTeams(vector<int>& v1) {
        ll ans=0;
        for(ll i=0;i<v1.sz();++i)
        {
            ll left1=0,right1=0;
            ll left2=0,right2=0;
            for(ll j=0;j<i;++j) {if(v1[j]<v1[i]) ++left1;else ++left2;}
            for(ll j=v1.sz()-1;j>i;--j) {if(v1[j]>v1[i]) ++right1;else ++right2;}
            ans+=left1*right1+left2*right2;
        }
        rn ans;
    }
};
