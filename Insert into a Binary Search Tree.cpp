...
class Solution {
public:
    TreeNode * fun(TreeNode *root,ll val)
    {
        if(!root) rn new TreeNode(val);
        if(val<root->val) root->left=fun(root->left,val);
        else if(val>root->val) root->right=fun(root->right,val);
        rn root;
    }
