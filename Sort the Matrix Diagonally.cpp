...
class Solution {
public:
    vector<vector<int>> diagonalSort(vector<vector<int>>& v1) {
        umap<ll,v<ll>> mp1;
    fori(0,v1.sz()) forj(0,v1[i].sz()) mp1[i-j].pb(v1[i][j]);
    for(auto &ele:mp1) sort(rng(ele.s),[](auto &a,auto &b){ rn a>b;});
    fori(0,v1.sz()) forj(0,v1[i].sz()){
        v1[i][j]=mp1[i-j].back();
        mp1[i-j].ppb();
    }
    rn v1;
